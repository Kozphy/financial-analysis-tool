...
def build_backtest_report(result: BacktestResult) -> str:
    ...
    lines = [
        ...
        f"Momentum lookback: {result.momentum_lookback_days} days",
        f"Volatility window: {result.volatility_lookback_days} days",
        f"Rebalance frequency: {result.rebalance_frequency}",
        f"Benchmark alignment: {result.benchmark_alignment}",
