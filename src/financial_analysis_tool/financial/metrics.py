...
from financial_analysis_tool.core.utils import safe_ratio
...
def analyze_records(records: list[FinancialRecord]) -> list[PeriodAnalysis]:
    if not records:
        raise InputDataError("At least one financial record is required for analysis.")
    ...
    for record in records:
        ...
        analyses.append(
            PeriodAnalysis(
                ...
                gross_margin=safe_ratio(gross_profit, record.revenue),
                operating_margin=safe_ratio(operating_income, record.revenue),
                net_margin=safe_ratio(record.net_income, record.revenue),
            )
        )
...
def summarize_company_performance(
    ...
    if not analyses:
        raise InputDataError("At least one analyzed period is required for summarization.")
    ...
    return PerformanceSummary(
        ...
        overall_revenue_growth=_calculate_growth(first_period.revenue, latest_period.revenue),
